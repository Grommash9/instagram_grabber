                ...
                if len(image_urls_list) < 13:
                    ...
                    try:
                        ...
                        for image_urls in image_urls_list:
                            if not image_urls_list.index(image_urls) > 20:
                                p = requests.get(image_urls)
                                with open(f'{config.results_storage_path}/{str(account_list.index(url))}/{str(image_urls_list.index(image_urls))}.jpeg', 'wb') as uploaded_image:
...
for accounts in account_list:
    ...
    result = get_dump(accounts)
    print(f'Аккаунт ({account_list.index(accounts)}/{len(account_list)}) {accounts} был {result} выгружен за {round(time.time() - start_time)} секунд')